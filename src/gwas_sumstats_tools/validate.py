...
class Validator(SumStatsTable):
    ...
    def schema(self) -> SumStatsSchema:
        effect_field = (
            self.effect_field()
            if self.effect_field() in SumStatsSchema().EFFECT_FIELD_DEFINITIONS
            else "beta"
        )
        p_value_field = (
            self.p_value_field()
            if self.p_value_field() in SumStatsSchema().PVALUE_FIELD_DEFINITIONS
            else "p_value"
        )
        schema = SumStatsSchema(
    ...
    def validate(self, full_file: bool = True) -> tuple[bool, str]:
        ...
        if self.valid:
            print("--> [green]Ok[/green]")
            print("Validating the chromosomes...")
        ...
        if self.valid:
            ...
            if not full_file:
                ...
                try:
                    df_iter = self.as_pd_df(chunksize=self.chunksize, skiprows=nrows)
    ...
    def _validate_field_order(self) -> tuple[bool, Union[str, None]]:
        ...
        if not valid:
            ...
            message = (
                "Fields not in the required order:\n"
    ...
    def _validate_chromosomes(self) -> tuple[bool, str]:
        ...
        if "chromosome" not in self.header():
            ...
            chr_series = pd.read_table(
                self.filename, sep=self.delimiter, usecols=["chromosome"], dtype=str
            )["chromosome"]
            ...
            if missing_optional:
                return (
                    True,
                    f"All autosomes exist. Optional chromosomes {missing_optional} do not exist.",
                )

            return True, "All chromosomes, including X, Y, and MT, exist."

    def _validate_df(
        self, dataframe: pd.DataFrame, message: str = "Data table is invalid"
    ) -> tuple[bool, str]:
        ...
        try:
            ...
            if len(failure_cases) > 0:
                ...
                failure_cases = failure_cases.sort_values(
                    by=["schema_context", "index", "column"], kind="mergesort"
                )
    ...
    def _evaluate_errors(self) -> None:
        ...
        if self.errors_table:
            ...
            if "DataFrameSchema" in self.errors_table["schema_context"]:
                ...
                if any(
                    row[0] == "Column"
                    and row[1] == "_p_value_mantissa"
                    and row[2] == "Must be greater than 0"
                    and row[4] == 0
...
def validate(
    ...
    full_file: bool = True,
) -> tuple[bool, str, Union[etl.Table, None], Union[str, None]]:
    ...
    if infer_from_metadata:
        ...
        if ssm:
            if pval_zero is False:
                pval_zero = (
                    True if ssm.as_dict().get("analysisSoftware") is not None else False
                )
        else:
            print(
