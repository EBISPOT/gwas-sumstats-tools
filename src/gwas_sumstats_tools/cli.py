...
def ss_validate(
    ...
    errors_file: bool = typer.Option(
        False,
        "--errors-out",
        "-e",
        help="Output erros to a csv file, <filename>.err.csv.gz",
    ),
    pval_zero: bool = typer.Option(
        False,
        "--p-zero",
        "-z",
    ...
    minimum_rows: int = typer.Option(
        100_000, "--min-rows", "-m", help="Minimum rows acceptable for the file"
    ),
    chunkzize: int = typer.Option(
        1_000_000,
        "--chunksize",
        "-s",
        help=(
            "Number of rows to store in memory at once. "
            "Increase this number for more speed at the cost "
            "of more memory. Decrease to save memory, at the "
            "cost of speed."
        ),
    ),
    infer_from_metadata: bool = typer.Option(
        False,
        "--infer-from-metadata",
        "-i",
        help=(
            "Infer validation options from the "
    ...
    with Progress(
        ...
        (valid, message, error_preview, error_type) = validate(
            filename=filename,
    ...
    print(message)
    if error_type:
        print((f"Primary reason for validation failure: [red]{error_type}[/red]"))
    if error_preview:
        print(
            (
                "See below for a preview of the errors. "
...
def ss_read(
    ...
    get_header: bool = typer.Option(
        False, "--get-header", "-h", help="Just return the headers of the file"
    ),
    metadata_infile: Path = typer.Option(
        None,
        "--meta-in",
        readable=True,
        exists=True,
        help=("Specify a metadata file to read in, defaulting to <filename>-meta.yaml"),
        show_default=False,
    ),
    get_all_metadata: bool = typer.Option(
        False, "--get-all-metadata", "-M", help="Return all metadata"
    ),
    get_metadata: Optional[List[str]] = typer.Option(
        None,
        "--get-metadata",
        "-m",
        help=(
            "Get metadata for the "
            "specified fields e.g. "
            "`-m genomeAssembly -m isHarmonised"
        ),
    ),
    remove_comments: str = typer.Option(
        None, "--remove_comments", "-r", help=("remove the comments in the file")
    ),
...
def ss_format(
    ...
    data_outfile: Path = typer.Option(
        None,
        "--ss_out",
        "-o",
        writable=True,
    ...
    config_outfile: Path = typer.Option(
        None,
        "--config_out",
        writable=True,
        file_okay=True,
    ...
    config_infile: Path = typer.Option(
        None,
        "--config_in",
        readable=True,
        exists=True,
    ...
    analysis_software: str = typer.Option(
        None,
        "--analysis_software",
        "-f",
        readable=True,
    ...
    minimal_to_standard: bool = typer.Option(
        False,
        "--minimal2standard",
        help=(
            "Try to convert a valid, minimally formatted file "
            ...
            "Please use '\t' for tab, ',' for comma, and "
            " for whitespace"
        ),
    ),
    generate_config: bool = typer.Option(
        ...
        "--generate_config",
        "-g",
        help=("To generate the configure file for the fileneeded to be formatted"),
    ),
    ...
    batch_apply: bool = typer.Option(
        None,
        "--batch_apply",
        "-b",
        help=("Apply configure files to the corresponding files"),
    ),
    lsf: bool = typer.Option(
        False, "--lsf", help=("running the batch process via subitting jobs via LSF")
    ),
    slurm: bool = typer.Option(
        False, "--slurm", help=("running the batch process via subitting job via Slurm")
    ),
...
def ss_gen_meta(
    ...
    generate_metadata: bool = typer.Option(
        False, "--generate-metadata", "-m", help="Create the metadata file"
    ),
    metadata_outfile: Path = typer.Option(
        None,
        "--meta-out",
        writable=True,
        file_okay=True,
    ...
    metadata_edit_mode: bool = typer.Option(
        False,
        "--meta-edit",
        "-e",
        help=(
    ...
    metadata_from_gwas_cat: bool = typer.Option(
        False,
        "--meta-gwas",
        "-g",
    ...
    """
    print(
        "[bold yellow]WARNING:[/bold yellow] This command is for dev internal use only and is not intended for end users."
    )
    meta_dict = (
        metadata_dict_from_args(args=extra_args.args) if metadata_edit_mode else {}
    )
    gen_meta(
        filename=filename,
