...
TEST_DATA = OrderedDict(
    {
        ...
        + [
            f"{chr}_{pos}_A_G"
            for chr, pos in zip(range(3, 25), range(135983, 135983 + 23))
        ],
        "rsid": ["rs185339560", "rs11250701", "rs12345", "rs7085086"]
        + [f"rs{i}" for i in range(1, 23)],
...
EFFECT_FIELDS = {
    "odds_ratio": [0.92090, 1.01440, 0.97385, 0.99302] + [0.99301] * 22,
...
class SSTestFile(TestFileBase):
    ...
    def __init__(
        ...
        sep: str = "\t",
        test_data: Union[OrderedDict, None] = None,
    ) -> None:
        self._setup_dir()
        self.filepath = os.path.join(TEST_DIR, filepath)
        self.sep = sep
        self.test_data = (
            OrderedDict(test_data)
            if test_data
            else OrderedDict({k: v for k, v in TEST_DATA.items()})
        )
    ...
    def replace_value(self, header: str, index: int, value: str) -> OrderedDict:
    ...
    def replace_header_and_data(
        self, data_to: list, header_from: str, header_to: str
    ) -> OrderedDict:
        self.test_data = OrderedDict(
            (header_to, data_to) if k == header_from else (k, v)
            for k, v in self.test_data.items()
        )
        return self.test_data

    def replace_header(self, header_from: str, header_to: str) -> OrderedDict:
        self.test_data = OrderedDict(
            (header_to if k == header_from else k, v) for k, v in self.test_data.items()
        )
        return self.test_data
...
class MetaTestFile(TestFileBase):
    ...
    def __init__(self, filepath: Path = "test_file.tsv-meta.yaml"):
        self._setup_dir()
        self.filepath = os.path.join(TEST_DIR, filepath)
