...
def test_append_to_path():
    ...
    assert append_to_path(path=test_path, to_add="-ext2.ext3") == Path(
        "./tests/utils_test/test.ext1-ext2.ext3"
    )
...
def test_replace_dictionary_keys():
    ...
    new_data_dict = replace_dictionary_keys(
        data_dict=data_dict, replace_dict=replace_dict
    )
